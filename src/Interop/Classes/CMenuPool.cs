...
using unsafe CreateDelegate = delegate* unmanaged[Thiscall]<CMenuPool*, byte, byte*, float, float, byte, float, float, CMenuInteraction*, CMenu*>;
...
public unsafe ref struct CMenuPool
{
    ...
    public CMenu* Create(byte menuId, string title, float x, float y, byte columns, float firstColumnWidth, float secondColumnWidth, in CMenuInteraction interaction)
    {
        ...
        return _create(RequireInstance(), menuId, titleAnsi, x, y, columns, firstColumnWidth, secondColumnWidth, &copy);
    }
